...
const Nav = () => {
  ...
  const [hoverEffect, setHoverEffect] = useState({ active: false, x: 0, width: 0 });
  ...
  return (
    <header 
      className={`fixed w-full top-0 left-0 transition-all duration-300 z-50 
        ${scrolled 
          ? "py-1.5 bg-navy/90 backdrop-blur-md" 
          : "py-2 bg-navy/85"}`}
      style={{
      ...
      <div className="container mx-auto px-4 flex justify-between items-center relative">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex flex-col">
            <span className="text-white font-medium tracking-wide text-sm" style={{
              textShadow: '0 0 8px rgba(0, 210, 255, 0.7)'
            }}>
              ELECTRONIC<span className="text-mint-green">_PROJECTS</span>
            </span>
            <span className="text-electric-blue/80 text-[10px] tracking-widest">CIRCUIT • DESIGN • BUILD</span>
          </div>
        </div>
        ...
        <nav className="hidden lg:flex items-center">
          <ul className="flex gap-6 list-none p-0 m-0 relative">
            {/* Navigation Items */}
            <NavItem href="/" label="HOME" isActive={pathname === "/"} onHover={setHoverEffect} />
            <NavItem href="/projects" label="PROJECTS" isActive={pathname.includes("/projects")} onHover={setHoverEffect} />
            <NavItem href="/students" label="STUDENTS" isActive={pathname.includes("/students")} onHover={setHoverEffect} />
            <NavItem href="/teams" label="TEAMS" isActive={pathname.includes("/teams")} onHover={setHoverEffect} />
            <NavItem href="/about" label="ABOUT" isActive={pathname.includes("/about")} onHover={setHoverEffect} />
            <NavItem href="/contact" label="CONTACT" isActive={pathname.includes("/contact")} onHover={setHoverEffect} />
...
const NavItem = ({ href, label, isActive, onHover }) => {
  ...
  const handleMouseEnter = () => {
    if (itemRef.current) {
      ...
      onHover({ 
        active: true, 
        x: rect.left - (itemRef.current.offsetParent?.getBoundingClientRect().left || 0),
        width: rect.width 
      });
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {isActive && (
          <div className="absolute inset-0 rounded-sm -mx-1 z-[-1]" style={{
            background: 'linear-gradient(180deg, rgba(0, 255, 216, 0.08) 0%, rgba(0, 255, 216, 0.15) 100%)',
        ...
        {isActive && (
          <div className="absolute -top-[1px] w-full h-[2px] overflow-hidden">
            <div className="w-full h-full bg-mint-green" style={{ 
